...
// GET search accounts by partial account number
router.get('/search', async (req, res) => {
    try {
        ...
        const results = await pool.query(
            `SELECT a.id, a.account_number, a.account_type, a.status, a.balance,
                    c.name as customer_name, c.email as customer_email
             FROM accounts a
             JOIN customers c ON a.customer_id = c.id
             WHERE CAST(a.account_number AS TEXT) ILIKE $1
